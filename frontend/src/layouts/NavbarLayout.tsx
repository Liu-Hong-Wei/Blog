    ...
    <>
      <nav className={`h-16 fixed top-0 w-screen text-xl bg-white/90 backdrop-blur-xl flex justify-center transition-shadow duration-300 ${scrolled ? 'shadow-md' : ''}`}>
        <div className="w-2xl h-full flex justify-center">
          ...
          {/* 菜单按钮，仅在小屏显示 */}
          {/* 原有NavLink，仅在md及以上显示 */}
          <ul className="hidden md:flex w-3xs md:w-xs h-full *:grow *:rounded-lg *:p-2 justify-start text-center items-center gap-4 *:hover:bg-slate-100 *:hover:duration-200">
            {[
              { to: "/posts", text: "Posts" },
              { to: "/ideas", text: "Ideas" },
          ...
          <aside className="fixed top-0 left-0 h-full w-48 bg-white z-50 shadow-lg flex flex-col transition-transform duration-300">
            ...
            <ul className="h-full flex flex-col p-4">
              {[
                ...
                <li className="pl-4 py-4 border-t-2 border-gray-200" key={to}>
                  <NavLink
                    to={to}
                    className={({ isActive }) =>
                      isActive ? "text-blue-400 font-medium" : "text-slate-700"
                    }
                    onClick={() => setDrawerOpen(false)}
                  >
                    {text}
