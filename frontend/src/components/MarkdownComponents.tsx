...
export const markdownComponents: Partial<Record<string, ElementType>> = {
  h1: ({ className, ...props }: ComponentProps<'h1'>) => (
    <h1 {...props} className={mergeClassName('mt-8 mb-4 border-b-2 border-bgsecondary pb-2 text-4xl font-bold', className)} />
  ),
  h2: ({ className, ...props }: ComponentProps<'h2'>) => (
    <h2 {...props} className={mergeClassName('mt-6 mb-3 border-b-2 border-bgsecondary pb-2 text-3xl font-semibold', className)} />
  ),
  ...
  h5: ({ className, ...props }: ComponentProps<'h5'>) => (
    <h5 {...props} className={mergeClassName('mt-5 mb-3 text-lg font-semibold uppercase tracking-wide text-secondary', className)} />
  ),
  h6: ({ className, ...props }: ComponentProps<'h6'>) => (
    <h6 {...props} className={mergeClassName('mt-4 mb-2 text-base font-semibold uppercase tracking-widest text-secondary/80', className)} />
  ),
  ...
  blockquote: ({ className, ...props }: ComponentProps<'blockquote'>) => (
    <blockquote
      {...props}
      className={mergeClassName('my-6 rounded-xl border-l-4 border-secondary/40 bg-bgsecondary/20 px-4 py-3 text-lg italic text-primary shadow-sm', className)}
    />
  ),
  ul: ({ className, ...props }: ComponentProps<'ul'>) => (
    <ul {...props} className={mergeClassName('my-4 list-disc space-y-2 pl-6 marker:text-secondary', className)} />
  ),
  ol: ({ className, ...props }: ComponentProps<'ol'>) => (
    <ol {...props} className={mergeClassName('my-4 list-decimal space-y-2 pl-6 marker:font-semibold marker:text-secondary', className)} />
  ),
  ...
  sup: ({ className, ...props }: ComponentProps<'sup'>) => (
    <sup {...props} className={mergeClassName('ml-0.5 text-xs align-super', className)} />
  ),
  ...
  figcaption: ({ className, ...props }: ComponentProps<'figcaption'>) => (
    <figcaption {...props} className={mergeClassName('mt-2 text-sm italic text-primary/70', className)} />
  ),
